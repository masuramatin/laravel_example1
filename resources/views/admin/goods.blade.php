				   ...
				   @foreach($hm_cleaning_supp as $passport)
					<div class="col-md-4">
					  ...
					  <img src="{{ url('/') }}/img/{{$passport->goods_image}}" alt="Lights" style="width:100%">
					    <div class="caption">
						<h3 align="center"><a href="/view_goods/{{$passport->id}}">{{$passport->item_name}}</a></h3>
						<h4 align="center"><a href="/view_goods/{{$passport->id}}">{{$passport->descripation}}</a></h4>
					    </div>
				     </div>
				   </div>
					 ...
					 <div class="panel-body">		 
					  <div id="demo1" class="collapse">
						<div class="form-group">
						 <div class="row">
							<div class="col-sm-1">
								<label class="form-group" style="font-size:20px">Search</label> 						   </div>
						   <div class="col-sm-2">
  ...
  <!-- Goods Distribution Start -->
       ...
       <tbody>  
			  ...
			  <tr>
				 ...
				 <td>{{$passport['descripation']}}</td>    
				 <td>
				 <a href="{{action('AdminController@view_dis', $passport['id'])}}" class="btn btn-warning">View</a><a href="{{action('AdminController@edit_dis', $passport['id'])}}" class="btn btn-warning">Edit</a>
				  
				  <form action="{{ route('dis.destroy', $passport['id']) }}" method="get">                  <input name="_method" type="hidden" value="DELETE">
            		<button class="btn btn-danger" type="submit">Delete</button>
      		</form>      
				</td>
		     </tr>
