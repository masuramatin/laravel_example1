    ...
    <hr />
      ...
      <div class="row">
      	<div class="col-md-3">
            ...
            @foreach($products as $passport)
            	@section('someSection')
          			{{ $order_date =  $passport->order_date }} 
        		@endsection
            @endforeach
            
    		<p>Oelivery Date: {{ $order_date }}</p>
    		<hr />            
         </div>
      	 <div class="col-md-3">
            @foreach($products as $passport)
            	@section('someSection')
          		{{ $delivery_date =  $passport->delivery_date }} 
        		@endsection
      	 ...
      	 <div class="col-md-3">
            ...
            @foreach($products as $passport)
            	@section('someSection')
          			{{ $number_of_cloth =  $passport->number_of_cloth }} 
        		@endsection
            
            @endforeach
            <p>Number of Cloth: {{ $number_of_cloth }}</p>
            <hr />
         </div>
      	 <div class="col-md-3">
            ...
            @foreach($products as $passport)
            	@section('someSection')
                	{{ $service_charge =  $passport->service_charge }} 
            	@endsection
            @endforeach
